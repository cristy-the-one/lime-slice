...
Contours are cut from a welded mesh and stitched by shared edge, not by rounded coordinates. A chain broken by a hole in the mesh is closed across gaps up to 2 mm, and overlapping shells are merged with a nonzero union instead of cancelling into holes. `slice --audit` reports the sliced volume against the mesh volume, repaired and dropped chains, and support volume that is inside the part or has nothing under it.
