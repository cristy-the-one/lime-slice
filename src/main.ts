...
app.innerHTML = `
  <div class="app">
    ...
    <footer class="status" id="status">Load an STL or 3MF. Arrow keys move the layer.</footer>
  </div>
`;
...
function estimateHtml() {
  ...
  const cost = ((est.filamentG / 1000) * state.pricePerKg).toFixed(2);
  return `
    <div class="meta"><b>${formatTime(est.seconds)}</b> · <b>${est.filamentG.toFixed(2)} g</b> · ${meters} m · €${cost}</div>
    <label class="field">Filament €/kg<input id="price" type="number" min="0" step="1" value="${state.pricePerKg}" /></label>
...
function onSettings(ev: Event) {
  ...
  const numIds = ["lh", "amin", "amax", "pa", "la", "zhopht", "zhopmin", "scarflen", "scarfsteps", "sangle", "shmult", "pastart", "paend", "pastep"] as const;
...
document.querySelector("#export")!.addEventListener("click", () => {
  ...
  download(state.result.gcode, `${base}.gcode`);
});
...
function setView(mode: typeof state.viewMode) {
  ...
  document.querySelectorAll<HTMLButtonElement>(".mode").forEach((el) => {
...
async function runSlice() {
  ...
  try {
    ...
    if (tauri) {
      ...
      const json = await invoke<string>("slice_model", { payload: JSON.stringify({ ...payload(), dataB64: "" }) });
      unlisten();
      body = await parseInWorker(id, json);
    } else {
      body = await postSlice(id, state.mesh.bytes, payload());
    }
    if (id !== job) return;
