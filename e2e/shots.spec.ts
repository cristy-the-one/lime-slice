...
test("ui states from real slice fixtures", async ({ page }) => {
  ...
  await shot(page, "02-loaded.png");

  await page.getByRole("button", { name: /^Speed/ }).click();
  await page.locator("#right").screenshot({ path: path.join(out, "03-card-speed.png") });
  await page.getByRole("button", { name: /^Efficiency/ }).click();
  await page.locator("#right").screenshot({ path: path.join(out, "04-card-efficiency.png") });
  await page.getByRole("button", { name: /^Toughness/ }).click();
  await page.locator("#right").screenshot({ path: path.join(out, "05-card-toughness.png") });
  await page.getByRole("button", { name: /^Speed/ }).click();
  ...
  await page.locator("#vslider").screenshot({ path: path.join(out, "06-layer-blend.png") });
  ...
  await page.locator("#view3d").screenshot({ path: path.join(out, "07-region-blend-plane.png") });
  ...
  await shot(page, "08-slicing-progress.png");
  await page.getByRole("button", { name: "Cancel" }).click();
  await expect(page.locator("#banner")).toContainText("cancelled");
  await shot(page, "09-cancelled.png");
  ...
  await page.locator("#view").screenshot({ path: path.join(out, "12-preview-2d.png") });
  await page.getByRole("button", { name: "Split", exact: true }).click();
  await page.waitForTimeout(300);
  await page.locator(".previews").screenshot({ path: path.join(out, "17-split.png") });
  await page.getByRole("button", { name: "3D", exact: true }).click();
  await page.waitForTimeout(400);
  await page.locator("#view3d").screenshot({ path: path.join(out, "13-preview-3d.png") });

  await page.locator("#legend label", { hasText: "Outer wall" }).locator("input").uncheck();
  await page.locator("#legend").screenshot({ path: path.join(out, "14-legend-toggles.png") });

  await page.locator("#estimate").scrollIntoViewIfNeeded();
  await page.locator("#estimate").screenshot({ path: path.join(out, "15-estimate-card.png") });

  await page.setViewportSize({ width: 960, height: 800 });
  await page.waitForTimeout(250);
  await shot(page, "16-narrow-960.png");
  await page.setViewportSize({ width: 1440, height: 900 });
